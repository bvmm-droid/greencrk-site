...
(function(){
  ...
  // ── Simple fixed admin toolbar ────────────────────────────────────────
  function bvmmAdminBar(){
    if(document.getElementById('bvmm-abar'))return;
    var hasToken=false;
    try{hasToken=!!sessionStorage.getItem('bvmm_token');}catch(_){}
    var isAdmin=window.location.search.indexOf('admin')>-1||hasToken;
    if(!isAdmin)return;
    var bar=document.createElement('div');
    bar.id='bvmm-abar';
    bar.style.cssText='position:fixed;top:44px;left:0;right:0;z-index:99999;background:#1a1200;display:flex;align-items:center;gap:4px;padding:4px 8px;overflow-x:auto;white-space:nowrap;-webkit-overflow-scrolling:touch;box-shadow:0 2px 8px rgba(0,0,0,.4)';

    function mkBtn(id,label,color){
      var lbl=document.createElement('label');
      lbl.id=id;
      lbl.style.cssText='cursor:pointer;padding:5px 12px;border-radius:6px;font-size:11px;font-weight:700;color:'+color+';border:1px solid '+color+';background:transparent;display:inline-block;flex-shrink:0';
      lbl.textContent=label;
      return lbl;
    }

    // Push Live button
    var plBtn=mkBtn('bvmm-abar-pl','🚀 Push Live','#4ade80');
    var plIn=document.createElement('input');plIn.type='file';plIn.accept='.html';plIn.style.display='none';
    plBtn.appendChild(plIn);
    plIn.onchange=function(ev){
      var file=ev.target.files[0];if(!file)return;
      var r=new FileReader();r.onload=function(e){
        var token=null;try{token=sessionStorage.getItem('bvmm_token');}catch(_){}
        if(!token){alert('Log in to admin first');return;}
        plBtn.textContent='⏳ Pushing...';plBtn.style.pointerEvents='none';
        fetch(WORKER,{method:'POST',headers:{'Content-Type':'application/json'},
          body:JSON.stringify({action:'admin-deploy',content:btoa(unescape(encodeURIComponent(e.target.result))),token:token,force:false})
        }).then(function(r){return r.json();}).then(function(d){
          plBtn.textContent=d.ok?'✅ Live!':'❌ Error';
          setTimeout(function(){plBtn.textContent='🚀 Push Live';plBtn.style.pointerEvents='';},3000);
        }).catch(function(){plBtn.textContent='❌ Error';setTimeout(function(){plBtn.textContent='🚀 Push Live';plBtn.style.pointerEvents='';},3000);});
      };r.readAsText(file);ev.target.value='';
    };
    bar.appendChild(plBtn);

    // Deploy Worker button
    var wkBtn=mkBtn('bvmm-abar-wk','⚙️ Worker','#f59e0b');
    var wkIn=document.createElement('input');wkIn.type='file';wkIn.accept='.js';wkIn.style.display='none';
    wkBtn.appendChild(wkIn);
    wkIn.onchange=function(ev){
      var file=ev.target.files[0];if(!file)return;
      var r=new FileReader();r.onload=function(e){
        var token=null;try{token=sessionStorage.getItem('bvmm_token');}catch(_){}
        wkBtn.textContent='⏳ Deploying...';wkBtn.style.pointerEvents='none';
        fetch(WORKER,{method:'POST',headers:{'Content-Type':'application/json'},
          body:JSON.stringify({action:'cf-deploy-worker',script:e.target.result,token:token})
        }).then(function(r){return r.json();}).then(function(d){
          wkBtn.textContent=d.ok?'✅ Deployed!':'❌ Error';
          setTimeout(function(){wkBtn.textContent='⚙️ Worker';wkBtn.style.pointerEvents='';},4000);
        }).catch(function(){wkBtn.textContent='❌ Error';setTimeout(function(){wkBtn.textContent='⚙️ Worker';wkBtn.style.pointerEvents='';},3000);});
      };r.readAsText(file);ev.target.value='';
    };
    ...
    siteA.href='https://greencrk.com';siteA.target='_blank';
    siteA.style.cssText='cursor:pointer;padding:5px 12px;border-radius:6px;font-size:11px;font-weight:700;color:#60a5fa;border:1px solid #60a5fa;background:transparent;display:inline-block;flex-shrink:0;text-decoration:none';
    siteA.textContent='🔗 View Site';
    bar.appendChild(siteA);

    document.body.appendChild(bar);
  }
  setTimeout(bvmmAdminBar,800);
  // Re-check after login
  var _origFetchAdmin=window.fetch;
