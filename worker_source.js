...
async function doFixRaceCondition(env) {
  ...
  const fixScript = `\n<script id="_bvmm_race_fix">\n(function(){\n  var sqData=null;\n  var isAdmin=false;\n\n  // Detect admin from DOM or session\n  try{\n    var _t=sessionStorage.getItem('bvmm_token');\n    if(_t) isAdmin=true;\n    else if(document.querySelector('.admin-bar')||document.body.classList.contains('admin-active')) isAdmin=true;\n  }catch(e){}\n\n  function esc(s){return (s||'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/\"/g,'&quot;').slice(0,200);}\n  function escQ(s){return (s||'').replace(/\\\\/g,'\\\\\\\\').replace(/'/g,\"\\\\'\").slice(0,200);}\n\n  function getToken(cb){\n    var t=sessionStorage.getItem('bvmm_token');\n    if(t){cb(t);return;}\n    // Auto-login using password prompt\n    var pw=prompt('Enter your admin password to save:');\n    if(!pw){cb(null);return;}\n    fetch('https://bvmm-proxy.babylonvillagemeatmarket.workers.dev',{\n      method:'POST',headers:{'Content-Type':'application/json'},\n      body:JSON.stringify({action:'login',password:pw})\n    }).then(function(r){return r.json();}).then(function(d){\n      if(d.ok&&d.token){sessionStorage.setItem('bvmm_token',d.token);cb(d.token);}\n      else{alert('Wrong password');cb(null);}\n    }).catch(function(){cb(null);});\n  }\n\n  function hideBTC(){\n    document.querySelectorAll('.mtab,.mpanel,.mgroup,.menu-tabs,.menu-tabs-wrap').forEach(function(el){\n      var sec=el.closest('section')||el.closest('.page-section');\n      if(sec) sec.style.display='none';\n      else el.style.display='none';\n    });\n  }\n\n  var pexCache={};\n  function loadPexels(name,imgEl){\n    if(pexCache[name]){imgEl.src=pexCache[name];imgEl.style.display='';if(imgEl.nextElementSibling)imgEl.nextElementSibling.style.display='none';return;}\n    fetch('https://bvmm-proxy.babylonvillagemeatmarket.workers.dev',{\n      method:'POST',headers:{'Content-Type':'application/json'},\n      body:JSON.stringify({action:'pexels',query:name+' meat butcher food',per_page:1})\n    }).then(function(r){return r.json();}).then(function(d){\n      var url=d.ok&&d.photos&&d.photos[0]?d.photos[0].src.large:null;\n      if(url){pexCache[name]=url;imgEl.src=url;imgEl.style.display='';if(imgEl.nextElementSibling)imgEl.nextElementSibling.style.display='none';}\n    }).catch(function(){});\n  }\n\n  var catMap={\n    'butcher - beef':'Beef','beef roast':'Beef','prime beef':'Beef','steak':'Beef','wagyu':'Beef','butcher shop':'Beef',\n    'butcher - veal':'Pork & Veal','butcher - pork':'Pork & Veal','veal':'Pork & Veal','veal & pork':'Pork & Veal',\n    'butcher - lamb':'Lamb',\n    'butcher - poultry':'Poultry','poultry':'Poultry',\n    'butcher - seafood':'Seafood','fish':'Seafood','frozen sea food':'Seafood',\n    'cheese & deli':'Deli','deli meats & cheeses':'Deli','cold salads':'Deli',\n    'sandwiches':'Sandwiches','heroes breads & wraps':'Sandwiches','paninis':'Sandwiches','lunch':'Sandwiches',\n    'soups':'Soups',\n    'pasta & eggplant':'Prepared','prepared foods':'Prepared','family dinners':'Prepared',\n    'vegetables potatoes & rice':'Prepared','appetizers':'Prepared','desserts':'Prepared','coffee':'Prepared','the regulars':'Prepared',\n    'catering':'Catering','all catering':'Catering','quick catering':'Catering',\n    'specials':'Specials','special orders':'Specials','dartagnan':'Specials','market place':'Specials','gear':'Specials',\n    'other':'Other'\n  };\n  var tabOrder=['Beef','Pork & Veal','Lamb','Poultry','Seafood','Deli','Sandwiches','Soups','Prepared','Catering','Specials','Other'];\n  function mapCat(c){return catMap[(c||'').toLowerCase()]||'Other';}\n  function slug(s){return (s||'').toLowerCase().replace(/\\s*[&\\/]\\s*/g,'-').replace(/\\s+/g,'-').replace(/[^a-z0-9-]/g,'');}\n\n  function adminBtns(item){\n    if(!isAdmin) return '';\n    return '<div style=\"position:absolute;top:8px;right:8px;display:flex;gap:6px;z-index:10\">'\n      +'<button onclick=\"event.stopPropagation();bvmmEditPhoto(\\''+escQ(item.id)+'\\',\\''+escQ(item.name)+'\\')\" style=\"width:32px;height:32px;border-radius:50%;background:rgba(255,255,255,0.92);border:none;cursor:pointer;font-size:15px\">📷</button>'\n      +'<button onclick=\"event.stopPropagation();bvmmEditItem(\\''+escQ(item.id)+'\\',\\''+escQ(item.name)+'\\',\\''+escQ(item.price||'')+'\\',\\''+escQ(item.desc||'')+'\\')\" style=\"width:32px;height:32px;border-radius:50%;background:rgba(255,255,255,0.92);border:none;cursor:pointer;font-size:15px\">✏️</button>'\n      +'</div>';\n  }\n\n  function buildFilters(usedGroups){\n    var el=document.querySelector('.og-filter');\n    if(!el) return;\n    var html='<button class=\"og-ftab active\" data-filter=\"all\">All Items</button>';\n    tabOrder.forEach(function(g){if(usedGroups[g])html+='<button class=\"og-ftab\" data-filter=\"'+slug(g)+'\">'+g+'</button>';});\n    el.innerHTML=html;\n    document.querySelectorAll('.og-ftab').forEach(function(btn){\n      btn.onclick=function(){\n        document.querySelectorAll('.og-ftab').forEach(function(b){b.classList.remove('active');});\n        btn.classList.add('active');\n        var f=btn.getAttribute('data-filter');\n        document.querySelectorAll('.bvmm-card').forEach(function(c){c.style.display=(f==='all'||c.getAttribute('data-cat')===f)?'':'none';});\n      };\n    });\n  }\n\n  function buildOG(data){\n    var grid=document.querySelector('.og-grid');\n    if(!grid) return;\n    var g=data.grouped||{};\n    var cats=Object.keys(g).sort();\n    var items=[],usedGroups={};\n    cats.forEach(function(cat){(g[cat]||[]).forEach(function(i){var group=mapCat(cat);usedGroups[group]=true;items.push({i:i,cat:cat,group:group});});});\n    if(!items.length) return;\n\n    if(!document.getElementById('bvmm-card-css')){\n      var s=document.createElement('style');s.id='bvmm-card-css';\n      s.textContent='.bvmm-card{background:#fff;border-radius:12px;border:1px solid #e8e4de;overflow:hidden;cursor:pointer;position:relative;display:flex;flex-direction:column}'\n        +'.bvmm-card-img{width:100%;aspect-ratio:4/3;object-fit:cover;display:block}'\n        +'.bvmm-card-img-ph{width:100%;aspect-ratio:4/3;background:#f0ede8;display:flex;align-items:center;justify-content:center;color:#bbb;font-size:12px}'\n        +'.bvmm-card-body{padding:12px 14px 14px;flex:1;display:flex;flex-direction:column}'\n        +'.bvmm-card-cat{font-size:10px;font-weight:600;letter-spacing:1.2px;text-transform:uppercase;color:#999;margin:0 0 5px}'\n        +'.bvmm-card-name{font-size:16px;font-weight:600;color:#1a1a1a;margin:0 0 6px;line-height:1.3}'\n        +'.bvmm-card-desc{font-size:12px;color:#777;margin:0 0 10px;line-height:1.5;flex:1}'\n        +'.bvmm-card-price{font-size:15px;font-weight:600;color:#1a1a1a;margin:0}'\n        +'.og-filter{display:flex;flex-wrap:wrap;gap:8px;margin-bottom:24px}'\n        +'.og-ftab{padding:8px 16px;border-radius:40px;border:1.5px solid #d4cfc8;background:#fff;font-size:13px;font-weight:500;cursor:pointer;color:#555}'\n        +'.og-ftab.active{background:#1a1a1a;color:#fff;border-color:#1a1a1a}';\n      document.head.appendChild(s);\n    }\n\n    buildFilters(usedGroups);\n\n    var html='';\n    items.forEach(function(e){\n      var i=e.i,group=e.group,ck=slug(group);\n      var price=i.price?'$'+parseFloat(i.price).toFixed(2)+' /lb':'';\n      var imgHtml=i.imageUrl\n        ?'<img class=\"bvmm-card-img\" src=\"'+esc(i.imageUrl)+'\" loading=\"lazy\" alt=\"'+esc(i.name)+'\">'\n        :'<img class=\"bvmm-card-img\" data-pexels=\"'+esc(i.name)+'\" style=\"display:none\" alt=\"'+esc(i.name)+'\">'\n         +'<div class=\"bvmm-card-img-ph\">'+esc(group)+'</div>';\n      html+='<div class=\"bvmm-card\" data-id=\"'+esc(i.id)+'\" data-cat=\"'+esc(ck)+'\" onclick=\"bvmmOpen(\\''+escQ(i.id)+'\\')\">'\n        +adminBtns(i)+imgHtml\n        +'<div class=\"bvmm-card-body\">'\n        +'<p class=\"bvmm-card-cat\">'+esc(group)+'</p>'\n        +'<p class=\"bvmm-card-name\">'+esc(i.name)+'</p>'\n        +(i.desc?'<p class=\"bvmm-card-desc\">'+esc(i.desc)+'</p>':'<p class=\"bvmm-card-desc\"></p>')\n        +'<p class=\"bvmm-card-price\">'+price+'</p>'\n        +'</div></div>';\n    });\n    grid.innerHTML=html;\n    grid.querySelectorAll('[data-pexels]').forEach(function(img){loadPexels(img.getAttribute('data-pexels'),img);});\n    var first=document.querySelector('.og-ftab.active')||document.querySelector('.og-ftab');\n    if(first) first.click();\n  }\n\n  window.bvmmOpen=function(id){\n    if(!sqData) return;\n    var found=null,foundCat='',foundGroup='';\n    Object.keys(sqData.grouped||{}).forEach(function(cat){(sqData.grouped[cat]||[]).forEach(function(i){if(i.id===id){found=i;foundCat=cat;foundGroup=mapCat(cat);}});});\n    if(!found) return;\n    var existing=document.getElementById('bvmm-popup');\n    if(existing) existing.remove();\n    bvmmQtyVal=1; bvmmCurrentPrice=found.price?parseFloat(found.price):0;\n    var pop=document.createElement('div');\n    pop.id='bvmm-popup';\n    pop.style.cssText='position:fixed;inset:0;z-index:9999;background:rgba(0,0,0,0.55);display:flex;align-items:flex-end;justify-content:center';\n    var img=found.imageUrl?'<img src=\"'+esc(found.imageUrl)+'\" style=\"width:100%;height:240px;object-fit:cover;border-radius:20px 20px 0 0;display:block\">':'';\n    pop.innerHTML='<div style=\"background:#fff;border-radius:20px 20px 0 0;width:100%;max-width:520px;max-height:90vh;overflow-y:auto\">'\n      +img+'<div style=\"padding:20px 20px 36px\">'\n      +'<button onclick=\"document.getElementById(\\'bvmm-popup\\').remove()\" style=\"float:right;background:none;border:none;font-size:24px;cursor:pointer;color:#aaa;margin-top:-4px\">&times;</button>'\n      +'<p style=\"font-size:10px;font-weight:600;letter-spacing:1.2px;text-transform:uppercase;color:#999;margin:0 0 6px\">'+esc(foundGroup)+'</p>'\n      +'<p style=\"font-size:22px;font-weight:700;color:#1a1a1a;margin:0 0 8px;line-height:1.2\">'+esc(found.name)+'</p>'\n      +(found.desc&&found.desc!=='NEEDS_REVIEW'?'<p style=\"font-size:14px;color:#666;line-height:1.6;margin:0 0 16px\">'+esc(found.desc)+'</p>':'')\n      +'<p style=\"font-size:20px;font-weight:700;color:#1a1a1a;margin:0 0 20px\">'+(bvmmCurrentPrice?'$'+bvmmCurrentPrice.toFixed(2)+' /lb':'Price on request')+'</p>'\n      +'<div style=\"display:flex;align-items:center;gap:16px;margin-bottom:12px\">'\n      +'<div style=\"display:flex;align-items:center;border:2px solid #e0e0e0;border-radius:40px\">'\n      +'<button onclick=\"bvmmQty(-1)\" style=\"width:44px;height:44px;border:none;background:none;font-size:22px;cursor:pointer;color:#333\">&#8722;</button>'\n      +'<span id=\"bvmm-qty-display\" style=\"min-width:32px;text-align:center;font-size:18px;font-weight:700\">1</span>'\n      +'<button onclick=\"bvmmQty(1)\" style=\"width:44px;height:44px;border:none;background:none;font-size:22px;cursor:pointer;color:#333\">+</button>'\n      +'</div>'\n      +'<span style=\"font-size:13px;color:#888\">lbs</span>'\n      +'<span style=\"margin-left:auto;font-size:16px;font-weight:700;color:#1a1a1a\" id=\"bvmm-subtotal\">'+(bvmmCurrentPrice?'$'+bvmmCurrentPrice.toFixed(2):'')+'</span>'\n      +'</div>'\n      +'<button onclick=\"bvmmAddToOrder(\\''+escQ(found.id)+'\\',\\''+escQ(found.name)+'\\','+bvmmCurrentPrice+')\" style=\"width:100%;padding:16px;background:#1a1a1a;color:#fff;border:none;border-radius:12px;font-size:16px;font-weight:700;cursor:pointer\">Add to Order</button>'\n      +'</div></div>';\n    document.body.appendChild(pop);\n    pop.addEventListener('click',function(e){if(e.target===pop) pop.remove();});\n  };\n\n  var bvmmQtyVal=1,bvmmCurrentPrice=0;\n  window.bvmmQty=function(d){\n    bvmmQtyVal=Math.max(1,Math.min(99,bvmmQtyVal+d));\n    var el=document.getElementById('bvmm-qty-display');if(el)el.textContent=bvmmQtyVal;\n    var s=document.getElementById('bvmm-subtotal');if(s&&bvmmCurrentPrice)s.textContent='$'+(bvmmCurrentPrice*bvmmQtyVal).toFixed(2);\n  };\n  window.bvmmAddToOrder=function(id,name,price){\n    var pop=document.getElementById('bvmm-popup');if(pop)pop.remove();\n    if(typeof openOrderSheet==='function') openOrderSheet(name,price,'','');\n    bvmmQtyVal=1;\n  };\n\n  window.bvmmEditPhoto=function(id,name){\n    var inp=document.createElement('input');inp.type='file';inp.accept='image/*';\n    inp.onchange=function(){\n      var file=inp.files[0];if(!file) return;\n      var reader=new FileReader();\n      reader.onload=function(e){\n        var b64=e.target.result.split(',')[1];\n        getToken(function(token){\n          if(!token) return;\n          fetch('https://bvmm-proxy.babylonvillagemeatmarket.workers.dev',{\n            method:'POST',headers:{'Content-Type':'application/json'},\n            body:JSON.stringify({action:'square-img-upload',itemId:id,imageName:name,imageBase64:b64,imageMime:file.type||'image/jpeg',token:token})\n          }).then(function(r){return r.json();}).then(function(d){\n            if(d.ok){\n              if(sqData) Object.keys(sqData.grouped||{}).forEach(function(cat){(sqData.grouped[cat]||[]).forEach(function(i){if(i.id===id&&d.imageUrl)i.imageUrl=d.imageUrl;});});\n              buildOG(sqData);alert('Photo updated!');\n            } else {alert('Error: '+(d.error||'Unknown'));}\n          }).catch(function(e){alert('Upload failed: '+e.message);});\n        });\n      };\n      reader.readAsDataURL(file);\n    };\n    inp.click();\n  };\n\n  window.bvmmEditItem=function(id,name,price,desc){\n    var ex=document.getElementById('bvmm-edit-modal');if(ex)ex.remove();\n    var modal=document.createElement('div');\n    modal.id='bvmm-edit-modal';\n    modal.style.cssText='position:fixed;inset:0;z-index:9999;background:rgba(0,0,0,0.55);display:flex;align-items:center;justify-content:center;padding:20px';\n    var cleanDesc=(desc==='NEEDS_REVIEW'?'':desc);\n    modal.innerHTML='<div style=\"background:#fff;border-radius:16px;width:100%;max-width:460px;padding:24px\">'\n      +'<p style=\"font-size:17px;font-weight:700;margin:0 0 16px\">Edit Item</p>'\n      +'<label style=\"font-size:12px;color:#888;display:block;margin-bottom:4px\">Item Name</label>'\n      +'<input id=\"bvmm-edit-name\" type=\"text\" value=\"'+esc(name)+'\" style=\"width:100%;padding:10px;border:1.5px solid #ddd;border-radius:8px;font-size:15px;margin-bottom:12px;box-sizing:border-box\">'\n      +'<label style=\"font-size:12px;color:#888;display:block;margin-bottom:4px\">Price per lb ($)</label>'\n      +'<input id=\"bvmm-edit-price\" type=\"number\" step=\"0.01\" value=\"'+esc(price)+'\" style=\"width:100%;padding:10px;border:1.5px solid #ddd;border-radius:8px;font-size:15px;margin-bottom:12px;box-sizing:border-box\">'\n      +'<label style=\"font-size:12px;color:#888;display:block;margin-bottom:4px\">Description</label>'\n      +'<textarea id=\"bvmm-edit-desc\" rows=\"3\" style=\"width:100%;padding:10px;border:1.5px solid #ddd;border-radius:8px;font-size:14px;resize:vertical;box-sizing:border-box\">'+esc(cleanDesc)+'</textarea>'\n      +'<div style=\"display:flex;gap:10px;margin-top:16px\">'\n      +'<button onclick=\"document.getElementById(\\'bvmm-edit-modal\\').remove()\" style=\"flex:1;padding:12px;border:1.5px solid #ddd;background:#fff;border-radius:8px;font-size:15px;cursor:pointer\">Cancel</button>'\n      +'<button id=\"bvmm-save-btn\" onclick=\"bvmmSaveItem(\\''+escQ(id)+'\\')\" style=\"flex:1;padding:12px;background:#1a1a1a;color:#fff;border:none;border-radius:8px;font-size:15px;font-weight:700;cursor:pointer\">Save to Square</button>'\n      +'</div></div>';\n    document.body.appendChild(modal);\n    modal.addEventListener('click',function(e){if(e.target===modal)modal.remove();});\n  };\n\n  window.bvmmSaveItem=function(id){\n    var nameVal=(document.getElementById('bvmm-edit-name')||{value:''}).value;\n    var priceVal=document.getElementById('bvmm-edit-price').value;\n    var descVal=document.getElementById('bvmm-edit-desc').value;\n    var priceCents=priceVal?Math.round(parseFloat(priceVal)*100):null;\n    var btn=document.getElementById('bvmm-save-btn');\n    if(btn)btn.textContent='Saving...';\n    getToken(function(token){\n      if(!token){if(btn)btn.textContent='Save to Square';return;}\n      fetch('https://bvmm-proxy.babylonvillagemeatmarket.workers.dev',{\n        method:'POST',headers:{'Content-Type':'application/json'},\n        body:JSON.stringify({action:'square-item-update',id:id,name:nameVal,description:descVal,priceCents:priceCents,token:token})\n      }).then(function(r){return r.json();}).then(function(d){\n        if(d.ok){\n          if(sqData) Object.keys(sqData.grouped||{}).forEach(function(cat){(sqData.grouped[cat]||[]).forEach(function(i){\n            if(i.id===id){if(nameVal)i.name=nameVal;if(priceCents)i.price=(priceCents/100).toFixed(2);i.desc=descVal;}\n          });});\n          var modal=document.getElementById('bvmm-edit-modal');if(modal)modal.remove();\n          buildOG(sqData);\n        } else {if(btn)btn.textContent='Save to Square';alert('Error: '+(d.error||'Unknown'));}\n      }).catch(function(e){if(btn)btn.textContent='Save to Square';alert('Save failed: '+e.message);});\n    });\n  };\n\n  function rerender(){\n    hideBTC();\n    if(sqData) buildOG(sqData);\n    setTimeout(function(){document.querySelectorAll('.mgroup').forEach(function(g){g.style.display=g.querySelector('.mrow,.wagyu-card')?'':'none';});},400);\n  }\n\n  var _f=window.fetch;\n  window.fetch=function(){\n    var p=_f.apply(this,arguments);\n    try{\n      var b=arguments[1]&&arguments[1].body;\n      if(b){\n        var parsed=JSON.parse(b);\n        if(parsed.action==='square-catalog'){p.then(function(r){return r.clone().json();}).then(function(d){if(d&&d.ok){sqData=d;setTimeout(rerender,150);}}).catch(function(){});}\n        if(parsed.action==='login'&&parsed.password){p.then(function(r){return r.clone().json();}).then(function(d){if(d&&d.ok&&d.token){sessionStorage.setItem('bvmm_token',d.token);isAdmin=true;setTimeout(function(){if(sqData)buildOG(sqData);},200);}}).catch(function(){});}\n      }\n    }catch(e){}\n    return p;\n  };\n\n  // Check admin bar after DOM loads\n  document.addEventListener('DOMContentLoaded',function(){\n    try{if(document.querySelector('.admin-bar')||document.querySelector('#bvmm-admin')){isAdmin=true;}}catch(e){}\n  });\n})();\n\n</script>`;
