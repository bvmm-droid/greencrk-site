...
async function doFixRaceCondition(env) {
  ...
  // Remove old fix if present, then re-apply updated version
  if (html.includes('_bvmm_race_fix')) {
    html = html.replace(/<script id="_bvmm_race_fix">[\s\S]*?<\/script>/g, '');
  }

  const fixScript = `\n<script id="_bvmm_race_fix">\n(function(){\n  var sqData=null;\n\n  // ── Category key ──────────────────────────────────────────────\n  function catKey(c){\n    c=(c||'').toLowerCase();\n    if(c.indexOf('beef')>-1) return 'beef';\n    if(c.indexOf('pork')>-1||c.indexOf('veal')>-1||c.indexOf('lamb')>-1) return 'pork';\n    if(c.indexOf('chicken')>-1||c.indexOf('poultry')>-1||c.indexOf('turkey')>-1) return 'poultry';\n    if(c.indexOf('seafood')>-1||c.indexOf('fish')>-1||c.indexOf('shellfish')>-1) return 'seafood';\n    if(c.indexOf('sandwich')>-1||c.indexOf('deli')>-1) return 'deli';\n    if(c.indexOf('soup')>-1) return 'prepared';\n    if(c.indexOf('catering')>-1) return 'prepared';\n    if(c.indexOf('prepared')>-1) return 'prepared';\n    return 'other';\n  }\n\n  function esc(s){return (s||'').replace(/\\\\/g,'\\\\\\\\').replace(/'/g,\"\\\\'\").replace(/\"/g,'&quot;').slice(0,120);}\n\n  // ── Hide Behind the Counter section entirely ──────────────────\n  function hideBTC(){\n    // .menu-section or section containing .mtab tabs\n    document.querySelectorAll('.menu-section,.btc-section,.showcase-section').forEach(function(el){el.style.display='none';});\n    // Hide any section whose heading contains \"behind the counter\" or \"butcher shop\"\n    document.querySelectorAll('section,div').forEach(function(el){\n      var h=el.querySelector('h1,h2,h3,h4');\n      if(!h) return;\n      var t=h.textContent.toLowerCase();\n      if((t.indexOf('behind')>-1&&t.indexOf('counter')>-1)||(t.indexOf('butcher')>-1&&t.indexOf('shop')>-1)){\n        el.style.display='none';\n      }\n    });\n    // Also hide the tab-based menu panel area\n    var tabArea=document.querySelector('.menu-tabs-wrap,.menu-panel-wrap,.btc-wrap');\n    if(tabArea) tabArea.style.display='none';\n    // Hide mgroup/mtab sections\n    document.querySelectorAll('.mtab,.mpanel,.mgroup,.menu-tabs').forEach(function(el){\n      var parent=el.closest('section');\n      if(parent) parent.style.display='none';\n    });\n  }\n\n  // ── Pexels fallback for missing images ────────────────────────\n  var pexelsCache={};\n  function getPexelsPhoto(name, cb){\n    if(pexelsCache[name]){cb(pexelsCache[name]);return;}\n    var worker='https://bvmm-proxy.babylonvillagemeatmarket.workers.dev';\n    fetch(worker,{method:'POST',headers:{'Content-Type':'application/json'},\n      body:JSON.stringify({action:'pexels',query:name+' food meat',per_page:1})\n    }).then(function(r){return r.json();}).then(function(d){\n      var url=d.ok&&d.photos&&d.photos[0]?d.photos[0].src.medium:null;\n      pexelsCache[name]=url;\n      cb(url);\n    }).catch(function(){cb(null);});\n  }\n\n  // ── Build Order Online grid ───────────────────────────────────\n  function buildOG(data){\n    var grid=document.querySelector('.og-grid');\n    if(!grid) return;\n    var g=data.grouped||{};\n    var items=[];\n    Object.keys(g).forEach(function(cat){(g[cat]||[]).forEach(function(i){items.push({i:i,cat:cat});});});\n    if(!items.length) return;\n\n    var html='';\n    items.forEach(function(e){\n      var i=e.i,cat=e.cat,ck=catKey(cat);\n      var price=i.price?'$'+parseFloat(i.price).toFixed(2):'';\n      var unit=price?'/LB':'';\n      var imgHtml=i.imageUrl\n        ?'<img src=\"'+i.imageUrl+'\" style=\"width:100%;height:100%;object-fit:cover;position:absolute;inset:0\" loading=\"lazy\">'\n        :'<div class=\"og-vis-placeholder\" data-pexels=\"'+esc(i.name)+'\" style=\"width:100%;height:100%;display:flex;align-items:center;justify-content:center;background:#f0ede8\"><span style=\"font-size:11px;color:#999;text-align:center;padding:8px\">'+esc(cat)+'</span></div>';\n      html+='<div class=\"og-card\" data-cat=\"'+ck+'\" data-id=\"'+esc(i.id)+'\" data-name=\"'+esc(i.name)+'\" data-price=\"'+(i.price||'')+'\" data-cat-label=\"'+esc(cat)+'\">';\n      html+='<div class=\"og-vis og-art-'+ck+'\" style=\"position:relative;overflow:hidden;cursor:pointer\" onclick=\"bvmmOpenItem(\\''+esc(i.id)+'\\')\">';\n      html+=imgHtml;\n      html+='<span class=\"og-cat-chip chip-'+ck+'\">'+esc(cat)+'</span>';\n      html+='</div>';\n      html+='<div class=\"og-body\" style=\"cursor:pointer\" onclick=\"bvmmOpenItem(\\''+esc(i.id)+'\\')\">';\n      html+='<div class=\"og-name\">'+esc(i.name)+'</div>';\n      if(i.desc) html+='<div class=\"og-desc\">'+esc(i.desc)+'</div>';\n      html+='<div class=\"og-foot\"><span class=\"og-price\">'+price+'<span class=\"og-unit\">'+unit+'</span></span>';\n      html+='<button class=\"og-add\" onclick=\"event.stopPropagation();bvmmOpenItem(\\''+esc(i.id)+'\\')\">+</button>';\n      html+='</div></div></div>';\n    });\n    grid.innerHTML=html;\n\n    // Lazy-load Pexels for placeholder cards\n    grid.querySelectorAll('[data-pexels]').forEach(function(el){\n      var name=el.getAttribute('data-pexels');\n      getPexelsPhoto(name,function(url){\n        if(!url) return;\n        var img=document.createElement('img');\n        img.src=url;\n        img.style.cssText='width:100%;height:100%;object-fit:cover;position:absolute;inset:0';\n        img.loading='lazy';\n        el.parentNode.insertBefore(img,el);\n        el.remove();\n      });\n    });\n\n    // Wire filters\n    document.querySelectorAll('.og-ftab').forEach(function(btn){\n      btn.onclick=function(){\n        document.querySelectorAll('.og-ftab').forEach(function(b){b.classList.remove('active');});\n        btn.classList.add('active');\n        var cat=(btn.getAttribute('data-filter')||btn.textContent.trim().toLowerCase().replace(/\\s*[&\\/]\\s*/g,'-').replace(/\\s+/g,'-').replace(/[^a-z-]/g,''));\n        document.querySelectorAll('.og-card').forEach(function(c){\n          c.style.display=(cat==='all-items'||cat==='all'||!cat||c.getAttribute('data-cat')===cat)?'':'none';\n        });\n      };\n    });\n    var at=document.querySelector('.og-ftab.active')||document.querySelector('.og-ftab');\n    if(at) at.click();\n  }\n\n  // ── Item popup (Square-style) ─────────────────────────────────\n  window.bvmmOpenItem=function(id){\n    if(!sqData) return;\n    var found=null;\n    var foundCat='';\n    var g=sqData.grouped||{};\n    Object.keys(g).forEach(function(cat){\n      g[cat].forEach(function(i){if(i.id===id){found=i;foundCat=cat;}});\n    });\n    if(!found) return;\n\n    var price=found.price?parseFloat(found.price):0;\n    var existing=document.getElementById('bvmm-popup');\n    if(existing) existing.remove();\n\n    var pop=document.createElement('div');\n    pop.id='bvmm-popup';\n    pop.style.cssText='position:fixed;inset:0;z-index:9999;background:rgba(0,0,0,0.55);display:flex;align-items:flex-end;justify-content:center';\n    pop.innerHTML='<div style=\"background:#fff;border-radius:20px 20px 0 0;width:100%;max-width:500px;padding:24px;position:relative;max-height:90vh;overflow-y:auto\">'\n      +'<button onclick=\"document.getElementById(\\'bvmm-popup\\').remove()\" style=\"position:absolute;top:16px;right:16px;background:none;border:none;font-size:22px;cursor:pointer;color:#666\">&times;</button>'\n      +(found.imageUrl?'<img src=\"'+found.imageUrl+'\" style=\"width:100%;height:200px;object-fit:cover;border-radius:12px;margin-bottom:16px\">':'')\n      +'<div style=\"font-size:11px;font-weight:600;color:#888;text-transform:uppercase;letter-spacing:1px;margin-bottom:6px\">'+esc(foundCat)+'</div>'\n      +'<div style=\"font-size:22px;font-weight:700;color:#1a1a1a;margin-bottom:8px\">'+esc(found.name)+'</div>'\n      +(found.desc?'<div style=\"font-size:14px;color:#666;margin-bottom:16px;line-height:1.5\">'+esc(found.desc)+'</div>':'')\n      +'<div style=\"font-size:18px;font-weight:700;color:#8B6914;margin-bottom:20px\">'+(price?'$'+price.toFixed(2)+' /lb':'Market price')+'</div>'\n      +'<div style=\"display:flex;align-items:center;gap:12px;margin-bottom:20px\">'\n      +'<div style=\"display:flex;align-items:center;border:1.5px solid #ddd;border-radius:30px;overflow:hidden\">'\n      +'<button onclick=\"bvmmQty(-1)\" style=\"width:40px;height:40px;border:none;background:none;font-size:20px;cursor:pointer;color:#333\">−</button>'\n      +'<span id=\"bvmm-qty\" style=\"min-width:32px;text-align:center;font-weight:600;font-size:16px\">1</span>'\n      +'<button onclick=\"bvmmQty(1)\" style=\"width:40px;height:40px;border:none;background:none;font-size:20px;cursor:pointer;color:#333\">+</button>'\n      +'</div>'\n      +'<span style=\"font-size:13px;color:#888\">lbs</span>'\n      +'</div>'\n      +'<div style=\"font-size:13px;color:#666;margin-bottom:20px\">Subtotal: <strong id=\"bvmm-subtotal\">'+(price?'$'+price.toFixed(2):'—')+'</strong></div>'\n      +'<button onclick=\"bvmmAddToOrder(\\''+esc(found.id)+'\\',\\''+esc(found.name)+'\\','+price+')\" style=\"width:100%;padding:16px;background:#1a1a1a;color:#fff;border:none;border-radius:12px;font-size:16px;font-weight:600;cursor:pointer\">Add to Order</button>'\n      +'</div>';\n    document.body.appendChild(pop);\n    pop.addEventListener('click',function(e){if(e.target===pop) pop.remove();});\n  };\n\n  var _qty=1;\n  window.bvmmQty=function(delta){\n    _qty=Math.max(1,Math.min(99,_qty+delta));\n    var el=document.getElementById('bvmm-qty');\n    if(el) el.textContent=_qty;\n    // Update subtotal\n    var priceEl=document.querySelector('#bvmm-popup [style*=\"8B6914\"]');\n    if(priceEl){\n      var p=parseFloat(priceEl.textContent.replace(/[^0-9.]/g,''))||0;\n      var s=document.getElementById('bvmm-subtotal');\n      if(s&&p) s.textContent='$'+(p*_qty).toFixed(2);\n    }\n  };\n\n  window.bvmmAddToOrder=function(id,name,price){\n    _qty=1; // reset\n    var pop=document.getElementById('bvmm-popup');\n    if(pop) pop.remove();\n    // Hand off to existing order sheet\n    if(typeof openOrderSheet==='function'){\n      openOrderSheet(name,price,'','');\n    }\n  };\n\n  // ── Main rerender ─────────────────────────────────────────────\n  function rerender(){\n    hideBTC();\n    if(sqData) buildOG(sqData);\n    setTimeout(function(){\n      document.querySelectorAll('.mgroup').forEach(function(g){\n        g.style.display=g.querySelector('.mrow,.wagyu-card')?'':'none';\n      });\n    },400);\n  }\n\n  // ── Intercept fetch ───────────────────────────────────────────\n  var _f=window.fetch;\n  window.fetch=function(){\n    var p=_f.apply(this,arguments);\n    try{\n      var b=arguments[1]&&arguments[1].body;\n      if(b&&JSON.parse(b).action==='square-catalog'){\n        p.then(function(r){return r.clone().json();}).then(function(d){\n          if(d&&d.ok){sqData=d;setTimeout(rerender,150);}\n        }).catch(function(){});\n      }\n    }catch(e){}\n    return p;\n  };\n\n  // Reset qty when popup opens\n  document.addEventListener('click',function(){_qty=1;},true);\n})();\n\n</script>`;
